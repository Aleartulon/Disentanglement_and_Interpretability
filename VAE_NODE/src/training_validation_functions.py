...
def train_epoch(conv_encoder, f, conv_decoder, ma_mi, device, optim, training_data,loss_coeff,
 RK, k,start_backprop, dim_input, lambda_regularization, time_dependence_in_f, clipping, Auto_Encoder):
    ...
    for field, dt, param in training_data:
        ...
        optim.zero_grad()
        # do with dictionary
        l1,l2_TF,l2_AR,l3, _,regularization_latent  = loss_sup_mixed(conv_encoder, f, conv_decoder, field, dt, param ,ma_mi, device,loss_coeff, RK, k, start_backprop, dim_input,lambda_regularization,True, time_dependence_in_f, Auto_Encoder)
...
def valid_epoch(conv_encoder, f, conv_decoder, ma_mi, device, validation_data,loss_coeff, RK, k, start_backprop, dim_input, lambda_regularization, time_dependence_in_f, Auto_Encoder):
    ...
    with tc.no_grad():
        for field, dt, param in validation_data:
            ...
            l1,l2_TF,l2_AR,l3, l_final, regularization_latent  = loss_sup_mixed(conv_encoder, f, conv_decoder, field, dt, param ,ma_mi, device,loss_coeff, RK, k, start_backprop, dim_input, lambda_regularization, False, time_dependence_in_f, Auto_Encoder)
    ...
    return l1_loss/count, l1_loss_unnorm/count, l2_TF_loss/count, l2_AR_loss/count , l3_loss/count, regularization_loss/count, l_real/count , loss/count
...
def nn_training(conv_encoder ,f, conv_decoder, training_data, validation_data, ma_mi, device, optim, 
     ...
     time_of_AE, dim_input, lambda_regularization, lambda_regularization_strength, time_dependence_in_f, TBPP_dynamic, clipping, is_coupled, Auto_Encoder):
    ...
    if not checkpoint:
        ...
        for i in range(epochs):
            ...
            if i < time_of_AE: #use only AR
                train_l1_data, train_l2_TF_data, train_l2_AR_data, train_l3_data, train_regularization_data, train_loss_data = train_epoch(conv_encoder, f, conv_decoder, ma_mi, device, optim, training_data, [loss_coeff[0],0,0,0], RK, k, start_backprop, dim_input, 0, time_dependence_in_f, clipping, Auto_Encoder)
                valid_l1_data, valid_l1_unnorm_data, valid_l2_TF_data, valid_l2_AR_data, valid_l3_data, valid_regularization_loss, valid_real_data, valid_loss_data = valid_epoch(conv_encoder, f, conv_decoder, ma_mi, device, validation_data,[1,1,1,1], RK, k, start_backprop, dim_input, lambda_regularization, time_dependence_in_f, Auto_Encoder)
                ...
                loss_coeff_2 = loss_coeff[2] * AR_strength * full_training_count #increase dynamically the strength of the L2_AR loss term
                ...
                train_l1_data, train_l2_TF_data, train_l2_AR_data, train_l3_data, train_regularization_data , train_loss_data = train_epoch(conv_encoder, f, conv_decoder, ma_mi, device, optim, training_data,[loss_coeff[0],loss_coeff[1],loss_coeff_2,loss_coeff[3]], RK, k, start_backprop, dim_input,lambda_strength, time_dependence_in_f, clipping, Auto_Encoder)
                valid_l1_data, valid_l1_unnorm_data, valid_l2_TF_data, valid_l2_AR_data, valid_l3_data, valid_regularization_loss, valid_real_data, valid_loss_data = valid_epoch(conv_encoder, f, conv_decoder, ma_mi, device, validation_data,[1,1,1,1], RK, k,start_backprop, dim_input, lambda_strength, time_dependence_in_f, Auto_Encoder)
            ...
            print("Loss coefficient 2: " +str(loss_coeff_2) + ", Latent regularization strength: " +str(lambda_strength))
            print('Train_loss_data = ' + str(train_loss_data) + ', l1 train loss = ' +str(train_l1_data) + ', l2 TF train loss = ' + str(train_l2_TF_data)+ ', l2 AR train loss = ' + str(train_l2_AR_data)+ ', l3 train loss = ' + str(train_l3_data)+ ', train latent regularization = ' + str(train_regularization_data))
            print('Valid_loss_data = ' + str(valid_loss_data)+ ', valid Real loss = ' +str(valid_real_data)  + ', l1 valid loss = ' +str(valid_l1_data) +  ', l1 valid unnorm loss = ' +str(valid_l1_unnorm_data) + ', l2 valid TF loss = ' 
                  + str(valid_l2_TF_data)+ ', l2 valid AR loss = ' + str(valid_l2_AR_data)+ ', l3 valid loss = ' + str(valid_l3_data)+ ', valid latent regularization = ' + str(valid_regularization_loss))
            ...
            if not is_coupled[0] and is_coupled[1] == 'AE' and i >=time_of_AE:
                valid_loss_data = valid_l1_data + valid_regularization_loss
            ...
            if valid_loss_data < loss_value: #careful valid loss tot!!
                ...
                print('Models saved!')
                save_checkpoint(conv_encoder, f , conv_decoder, optim, scheduler, i, loss_value, loss_coeff_2 , lambda_strength, start_backprop, full_training_count,PATH+'/checkpoint/check.pt')
                early_stopping = 0
